...
#define copyarray(A, B)	\
	...
	A[14] = B[14];		\
	A[15] = B[15];
#define u8byte(a,i) ((u32)(a >> (i * 8)) & 0xff)
...
void MixColumns32_inv(u32 *s0, u32 *s1, u32 *s2, u32 *s3)
{
	int i;
	u8 temp[16], S[16];
	u4byte_out(S, *s0);
...
void GetMixInv() //복호화 최적화에 사용할 MixInv[] 생성
{
	int i;
	printf("u32 Td0[256] = {\n");
	for (i = 0; i < 256; i++)
...
/*MUL9(Sbox_inv[i] << 24) ^ (Sbox_inv[i] << 16) ^ (Sbox_inv[i] << 8) ^ Sbox_inv[i]
MULD(Sbox_inv[i] << 24) ^ (Sbox_inv[i] << 16) ^ (Sbox_inv[i] << 8) ^ Sbox_inv[i]
MULB(Sbox_inv[i] << 24) ^ (Sbox_inv[i] << 16) ^ (Sbox_inv[i] << 8) ^ Sbox_inv[i]
